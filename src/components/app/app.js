...
export default class App extends Component { //Компонент всего приложения
  maxId = 100;
  ...
  onToggleDone = (id) => {
    this.setState(({ todoData })=>{
      ...
      const newTask = { ...oldTask, //копируем весь объект
        done: !oldTask.done}; // заменяем свойство на противоположное !oldTask.done
  ...
  selectCompleted = () => {
    this.setState(({ todoData }) =>{
      ...
      for (let elem of newArr){
        ...
        if(!elem.done){// если Задача Активна
          elem.visible = false; // сделай ее невидимой
        } else {
          elem.visible = true; // иначе сделай видимой
        }
